...
- `boost where --diff=<name>` now builds its unified diff with `StrictUnifiedDiffOutputBuilder` instead of `UnifiedDiffOutputBuilder`, which `sebastian/diff` 9.0 removed. The `--- vendor:` / `+++ host:` header lines are now emitted by the builder itself via its `fromFile` / `toFile` options rather than written by hand, so the text of the diff is unchanged — the only visible difference is that those two lines are no longer dimmed. Only options present in both diff 7 and diff 9 are passed, so behaviour is identical on every supported major.
