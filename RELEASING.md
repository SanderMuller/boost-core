...
**The static `version` field is for Dependabot — do not remove it.** `composer.json` declares `"version": "1.4.0"`. boost-core dev-requires `sandermuller/boost-skills`, which in turn requires `sandermuller/boost-core: ^1.4` — a self-reference through the family. Composer cannot modify the root package, so the root must already satisfy `^1.4` or NOTHING resolves. Locally and in CI the `COMPOSER_ROOT_VERSION: dev-main` env var supplies that; Dependabot has no equivalent knob and resolves the root as `1.0.0+no-version-set`, which fails every update it attempts — not only a boost-skills one. The field is the only lever that reaches Dependabot.

It is deliberately the FLOOR (`1.4.0`), not the current version, so it needs no bump per release — any value satisfying boost-skills' `^1.4` works. Raise it only if boost-skills ever raises its boost-core floor above it. Packagist reads the git tag and ignores this field for published releases, so tagged versions are unaffected; `composer validate --strict` prints one warning about it, which is expected and is not run in CI.
