...
- `SanderMuller\BoostCore\Sync\SyncReporter::__construct(array $commandInvocations = [])` — map bare boost-core command names to how they are invoked in THIS project (`['tags' => 'php artisan project-boost:tags']`). The report contains follow-up advice; unmapped names fall back to `vendor/bin/boost <name>`, which is wrong in a wrapper's own output.
...
- `SanderMuller\BoostCore\Sync\SkillShipmentIndex::from(SyncResult)` — which skills a sync put on disk, plus the host-vs-vendor shadow maps. `isShipped()`, `shippedNames()`, `shadowedVendorFor()`, `guidelineShadowedVendorFor()`, `shadowedVendorMap()`, `guidelineShadowedVendorMap()`, `statusFor(string $skillName, list<string> $tags)`. It owns the INVERSE of `AgentTarget::skillRelativePathForName()`, so a wrapper never pattern-matches emit paths itself — only `skillsDirectoryRelative()` is frozen, not the fact that its value ends in `/skills`.
