...
final readonly class GuidanceComposer
{
    ...
    /**
     * Prepended to every NON-empty emitted guidance file (CLAUDE.md / AGENTS.md
     * / GEMINI.md). These are wholesale-regenerated each sync, so a hand-edit
     * here is silently overwritten — the banner says so and points at the
     * durable edit surfaces. Public so emission tests can assert it. Only added
     * when there IS content: an empty assembly stays `''` so the empty-assembly
     * guard in GuidanceWriter still distinguishes "nothing to emit".
     */
    public const string GENERATED_BANNER = '<!-- GENERATED by boost — do not edit. Change .ai/ (guidelines, skills) or .config/boost.php (conventions, agents, tags), then run `vendor/bin/boost sync`. Direct edits to this file are overwritten on the next sync. -->';
