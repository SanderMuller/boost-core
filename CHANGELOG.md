...
1.12.1 is the first installable 1.12 release. The 1.12.0 tag kept the
1.11.0 `version` field in `composer.json`, so Packagist skipped it. 1.12.1 has
the same code as 1.12.0, plus the corrected version field.

Claude Code guidance moves from `CLAUDE.md` to `AGENTS.md`. Claude Code reads
`AGENTS.md` natively since v2.1.277, and laravel/boost made the same change in
[laravel/boost#1032](https://github.com/laravel/boost/pull/1032). Read the
upgrade notes before you update: this release changes which files a sync
writes.

### Changed

- **Claude Code guidance is written to `AGENTS.md`.** Claude Code now shares
  the file with Codex, Amp, and the other agents that read it. boost-core no
  longer writes `CLAUDE.md`.
- **The `## Project Conventions` block moves with it.** It now renders into
  `AGENTS.md`, and it appears once when Claude Code and Codex share the file.
- **`convert-conventions` next steps** now point at `AGENTS.md` and say what to
  do with the remaining `CLAUDE.md`.

### Added

- **A warning when a `CLAUDE.md` blocks `AGENTS.md`.** By default Claude Code
  reads `AGENTS.md` only when the project has no `CLAUDE.md`,
  `.claude/CLAUDE.md`, or `CLAUDE.local.md`. `boost sync` and `boost doctor`
  now warn when such a file exists and boost emits Claude guidance. The warning
  stays quiet when the file imports `AGENTS.md` (`@AGENTS.md`, or
  `@../AGENTS.md` in `.claude/CLAUDE.md`).

### Upgrading

On the first sync after the update:

- A `CLAUDE.md` that boost wrote since 0.13 and that you did not edit is
  deleted. Commit the deletion together with the new or changed `AGENTS.md`.
- A `CLAUDE.md` you wrote or edited stays. So does a marker-bounded one from
  before 0.12, or any `CLAUDE.md` in a project with gitignore management off.
  Add an `@AGENTS.md` line to it, or move its content into `.ai/guidelines/`
  and delete it.
- Team members on a Claude Code version before v2.1.277 do not read
  `AGENTS.md`. Update Claude Code, or keep a `CLAUDE.md` that contains
  `@AGENTS.md`.

`UPGRADING.md` has the full steps.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.12.0...1.12.1

## [1.12.0](https://github.com/sandermuller/boost-core/compare/1.11.0...1.12.0) - 2026-09-24

<!-- verified-sha: eddea5bf76b58ba0ce3ee328157ead80fe810a86 -->
Claude Code guidance moves from `CLAUDE.md` to `AGENTS.md`. Claude Code reads
`AGENTS.md` natively since v2.1.277, and laravel/boost made the same change in
[laravel/boost#1032](https://github.com/laravel/boost/pull/1032). Read the
upgrade notes before you update: this release changes which files a sync
...
**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.11.0...1.12.0

## [1.11.0](https://github.com/sandermuller/boost-core/compare/1.10.1...1.11.0) - 2026-09-23

<!-- verified-sha: 414a91f406dbf6fc42a53f97a4b0c30f5d615a07 -->
A small minor release. It adds one tag to the `Tag` enum and widens the
documented meaning of `frontend`.

### Added

- **`Tag::Sentry`** (`'sentry'`). The capability tag for a project that tracks
  errors in Sentry and can reach them through a Sentry MCP server. boost-skills
  uses it for its Sentry research subagent. A `'sentry'` string tag already
  worked — boost-core never validates tags against the enum — so the case adds
  autocomplete, not new behavior.
  
  `boost.php` files that the config writer rewrites now render the tag as
  `Tag::Sentry`. Such a config needs boost-core `^1.11`.
  

### Changed

- **`frontend` now means "the project has a user-facing UI"**: templates,
  styles, JS, or any mix. It used to mean a JS toolchain. The JS checks skip
  what the project lacks, so a Blade-only or Livewire app can declare
  `frontend` and get the UI-facing skills. The tag catalog and the
  project-boost-laravel tag shapes now say this; the Livewire shape includes
  `'frontend'`.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.10.1...1.11.0

## [1.10.1](https://github.com/sandermuller/boost-core/compare/1.10.0...1.10.1) - 2026-09-16

<!-- verified-sha: ad70ed05de3fc3b20e965330cd0e952cea3cd765 -->
A patch release. Three fixes, all found by consumer usage of 1.10.0.

The load-bearing one: a project that registers its own skill renderer lost every
host subagent. Sync reported no error — the emit simply contained nothing from
`.ai/subagents/`. Any consumer with a renderer is affected, so a floor bump to
`^1.10.1` is warranted for packages that rely on host subagents reaching the
emit.
...
- A sync that merges an extra skill renderer dropped every host
  ...
  rebuilt the config without `subagentsPath`, so the path fell back to its empty
  default and the subagent pipeline read it as "no host subagents". The rebuild
  now carries the field, and a test asserts every config field survives the
  merge.
...
- `boost sync --scope=user` described itself as publishing skills only. The help
  text now names the guidelines half that 1.10.0 added, so `--help` stops
  contradicting the docs.
  

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.10.0...1.10.1
...
User scope published skills only. A package could ship a guideline that applies
everywhere — a voice rule, a verification rule — with no way to reach a session
outside a project.

Guidelines cannot follow the skills rule. A skill costs one description line
until it matches; a guideline is always-on, so publishing a package's set
wholesale would put project-specific instructions into every repository on the
machine. This release makes publication opt-in per guideline, declared by the
author.

### Added

- **User-scope guidelines.** An author marks a guideline machine-wide safe in
  `resources/boost/guidelines/.boost-user-scope.yaml` — a list of paths relative
  to the guidelines directory — or with `metadata.boost-user-scope: true` in
  frontmatter. The sidecar exists because a laravel/boost-safe package's
  guidelines stay frontmatter-free. One rule, two carriers.
- **One boost-owned file per package**, `~/.claude/boost/<vendor>__<package>.md`.
  Boost owns it wholesale, the same rule it applies at project scope, and never
  writes your own `~/.claude/CLAUDE.md` — the import line that activates the
  file stays yours. Claude Code is the only agent that opts in;
  `AgentTarget::userScopeGuidanceFileRelative()` defaults to `null`, so
  user-level import semantics are never guessed for an agent where they are
  unverified.
- **Eligibility is not a tag.** A tag scopes a guideline to projects and needs a
  `boost.php` to answer; user scope has none. The two sidecars answer different
  questions and never interact, so a guideline can be both tagged and
  user-scope eligible.
- **The file rides the existing per-package manifest.** Withdraw eligibility and
  it is reaped; remove the package and it is reaped; edit it and it is
  preserved. A run driving a narrowed agent set never reaps guidance for an
  agent it does not drive.

### Refused rather than published

A guideline reaches every session, so a doubtful one is not published at all:

- A **conventions token** cannot resolve at user scope — no `boost.php`, no
  conventions section — so the guideline is refused instead of shipping a raw
  token into every session.
- A selected guideline **no registered renderer can read** is refused, rather
  than publishing guidance that silently omits the content the author chose.
- A failed plan **publishes nothing at all**, skills included. An unclean run
  skips the ownership manifest update, and files written under a stale manifest
  could never be reaped.
- A render failure that concerns a guideline the sidecar does not select never
  blocks user-scope publication. It belongs to project scope alone.
- Both carriers fail closed: an unreadable sidecar, a sidecar that is not a
  list, and a `metadata.boost-user-scope` that is not `true` all mean "not
  eligible".

### Limitations

- A package must also ship `resources/boost/skills/`.
  `boost sync --scope=user --all` discovers and reaps packages by that
  directory, so a guidelines-only package is not seen. This widens when such a
  package appears.

### Documentation

- [Automating sync](https://sandermuller.github.io/boost-core/guide/automating-sync#user-scope-guidelines)
  carries the mechanism, the two-sidecar rule, and the worked example of a
  guideline that is both tagged and eligible.
- [File ownership](https://sandermuller.github.io/boost-core/guide/file-ownership)
  carries the ownership rule for the published file.

Nothing existing changes behaviour. A package that marks nothing eligible
publishes exactly what it published before.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.9.1...1.10.0
...
Some review passes only work in a fresh context. A pass that judges a diff as
somebody else's code cannot do that inline, where the same agent that wrote the
code applies the rules — flattening it into a skill keeps the rules and loses
the distrust that made them worth applying. Claude Code has subagents for
exactly this, and until now no package could ship one.

This release adds subagents as a first-class artefact beside skills, guidelines
and commands.

### Added

- Packages publish subagent definitions from `resources/boost/subagents/*.md`, overridable with `extra.boost.subagents`, and a project authors its own in `.ai/subagents/` (`withSubagentsPath(...)`). Both emit to `.claude/agents/boost/`, namespaced by provenance: `host/<name>.md` for yours, `<vendor__package>/<name>.md` for a package's. Subagents are a Claude Code concept, so every other target receives nothing and is not warned about it every run — `boost doctor` reports the skipped count once.
- boost claims only the `boost/` subtree, and only that subtree enters the managed `.gitignore` block. `.claude/agents/` itself holds definitions you wrote: they are never gitignored, never recorded in the manifest, and never removed by a sync. A project adopts a package's subagents one at a time with nothing to migrate first.
- Ingest follows the skill rules rather than the command ones — vendor and host sources, `boost-tags` filtering, `withExcludedSkills()` as the deny-list, host-over-vendor shadowing with the shadow reported by `boost where` and `boost doctor`, and vendor-vs-vendor collisions fatal unless `--force`.
- Skills declare a hard dependency with `metadata.boost-requires: "subagent:<name>"`. Bare names still mean skills, so a skill and a subagent may share one. A demanded subagent that tag filtering would have dropped is rescued exactly like a skill; an unsatisfiable demand warns whether the name was **excluded** by the project or **missing** from every installed package, and never fails the sync.
- `boost where` gains a SUBAGENTS category with shadowing annotated inline, and `boost doctor` gains a Subagents section: unmet dependencies, sources it skipped, targets that receive nothing, and definitions that share a `name`.
- A subagent's identity is its `name` frontmatter and never its path, so two files can claim one name from anywhere under `.claude/agents/` — including subfolders — and Claude Code loads one of them by filesystem read order. `boost doctor` lists every such overlap it finds, and `boost sync` warns when a name it is about to emit is also claimed by a file it does not own. boost never picks a winner: at least one file is normally yours.

### Compatibility

- `BoostRequires::parse()` keeps returning bare skill names only. A `subagent:` token never reaches it, so a wrapper package that predates this release cannot receive one and report it as a missing skill. `parseSubagents()` returns the subagent demands. An unknown prefix is malformed: a sync warning, and an error under `boost validate --strict`.
- **A package declaring a `subagent:` dependency must floor `boost-core` at 1.9.0.** An older engine reads the whole token as a skill name and warns that it is missing, and no change here can fix that retroactively. The subagent *files* need no floor — an older engine never looks for `resources/boost/subagents/`, so a package can ship them today and add the requires when it raises its floor.
- `Skill` gains an appended `requiredSubagents`, and `BoostConfig` an appended `subagentsPath`, both per the existing additive rules for those constructors.

### Fixed

- A file inside a boost-owned subagent subtree is preserved by the stale-file sweep even when no manifest exists — the one managed directory that is not clean-slate. `.claude/agents/` is a directory operators write into, so a wiped `.boost/` must not cost them files boost never wrote. Every other managed directory is unchanged.
- A project whose only artefact is a subagent wrote `.boost/manifest.json` without adding `.boost/` to the managed `.gitignore` block.

### Documentation

- [Subagents](https://sandermuller.github.io/boost-core/guide/subagents) covers the layout, the ownership boundary, the collision rules, and how to write a skill that dispatches a subagent when present and says what the inline fallback loses when it is not. That last part matters: a weaker pass that does not announce itself is read as a clean one.
- The same page states what a capability claim in the frontmatter is worth. boost passes `tools` and `disallowedTools` through without validating them, so the definition is the only place the claim is made — and `disallowedTools` denies the tools it names and nothing else. A definition that grants `Bash`, denies `Write, Edit`, and then opens "you are read-only" describes a fence that is not there.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.8.1...1.9.0

## [1.8.1](https://github.com/sandermuller/boost-core/compare/1.8.0...1.8.1) - 2026-08-29

<!-- verified-sha: 2d94a989ee9bc2efa1060805ae7983bf9d260513 -->
**1.8.0 is not installable.** Packagist skipped that tag over a `composer.json`
version mismatch, so the tag and its GitHub release exist while `composer require` cannot resolve them. 1.8.1 is the first installable release of the
1.8 line and carries everything 1.8.0 described, plus the fixes below. Upgrade
straight to it.

### Fixed

- `composer.json` carries a static `version` field so Dependabot can resolve the root package. It was introduced after 1.7.0 and pinned at `1.4.0` on the belief that Packagist ignores the field for tagged releases. It does not — it compares the field to the tag and skips the tag on a mismatch, reporting it only in the Packagist update log: `Skipped tag 1.8.0, tag (1.8.0.0) does not match version (1.4.0.0) in composer.json`. The field now tracks each release, and a tag-push workflow fails when the two disagree rather than leaving Packagist to skip in silence.
- `extra.branch-alias.dev-main` was `1.4.x-dev`, four minors behind. Anything resolving `dev-main` — a consumer pinning `^1.8@dev`, or a sibling package following the branch — got an alias claiming 1.4 and could not satisfy its own constraint. The same workflow now checks the alias against the tag's minor line.
- The entry-point banner no longer fires at the root of the package that declares it. 1.8.0 honoured `extra.boost.entry-point` from any installed package, the root included. That reads as useful self-protection only where the root is an application: at the root of a package that *ships* a wrapper there is no application, so the named `php artisan …` invocation cannot run at all, while bare `vendor/bin/boost` is the correct command there. A package's map describes projects that install it, so its own claim is ignored — and `boost doctor` reports the ignored claim rather than dropping it silently.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.8.0...1.8.1
...
A project that installs a wrapper package has two entry points, and the bare
binary was silent about the difference. Bare `sync` composed guidance without
the wrapper's injected skills and exited 0; bare `tags` listed a short set. This
release makes `vendor/bin/boost` say what it is not running, and fixes five
defects where a failure channel was populated and never read.

### Added

- Wrapper packages declare the commands they cover in `composer.json` under `extra.boost.entry-point`, mapping a bare command name to the invocation to run instead. `vendor/bin/boost` prints that on stderr, so a `--json` envelope on stdout stays machine-readable. A command the wrapper does not cover still runs, and says its result is incomplete.
- `BOOST_STRICT_ENTRY_POINT=1` turns that advisory into a refusal. Opt-in for all of 1.x — CLI exit codes are inside the SemVer promise, so a previously-passing command cannot start failing in a minor. It becomes the default in the next major.
- `doctor` is reserved: a wrapper cannot claim it, because it is what diagnoses a wrapper whose own CLI will not boot. `boost doctor` lists every declared entry point, warns about a rejected claim, and names a command two packages both claim.
- `SyncReporter` renders a completed sync exactly as `bin/boost sync` does. `render()` returns a `SyncReportOutcome` — `hasErrors`, `hasConventionsError`, `hasTokenLeak`, `hasDrift`, plus the `exitCode` boost-core would use — so a package with its own documented exit contract renders identically and still decides for itself. `report()` is that call with `->exitCode` taken.
- `SyncSummary` names the `wrote=/unchanged=/deleted=` line that `BoostAutoSync` already parses to decide whether a `post-install-cmd` stays silent. A wrapper emitting it is parsed by the same hook.
- `SkillShipmentIndex` and `SkillShipmentStatus` answer "did this skill ship, and if not why" from a `SyncResult`. The index owns the inverse of `AgentTarget::skillRelativePathForName()`, so a caller never pattern-matches emit paths — only `skillsDirectoryRelative()` is frozen, not the fact that its value ends in `/skills`.

### Fixed

- `boost scan`, `install` and `remote` wrote `boost.php` with nobody prompted. The picker guard checked Symfony's `--no-interaction` flag, which is the only thing that clears it; laravel/prompts probes the terminal itself and silently returns its precomputed default when there is none. In the gap, any non-TTY run that did not pass `-n` — CI, git hooks, Composer scripts, agent shells — fell through to the picker. **This changes an exit code**: those three commands now exit 1 with guidance where they previously "succeeded" unattended.
- `boost doctor` reported "Generated files match sources" over a run that failed. A source whose renderer throws is excluded by the loader, so the drift check compared a truncated source set and found nothing to change. Doctor now consults `SyncResult::hasErrors()` and refuses to give a verdict rather than giving a wrong one.
- `boost sync` exited 1 with an empty screen when an emitter failed on an otherwise clean run. `hasErrors()` is true for an `ERRORED` emitter, but the errors list does not contain one, and only the list was rendered.
- `boost scan` could not keep an allowlisted vendor it was unable to rediscover. The picker offered only what `VendorScanner` found while the writer replaced `withAllowedVendors()` wholesale, so an entry whose content appears later than a bare scan can see was impossible to keep — not merely deselected. Such entries are now listed, labelled and preselected.
- `boost scan` appended `->withDisabledEmitters([])` to any config that never declared it.
- A host copy shadowing the same name across several vendors reported one of them. `boost where` now names all of them.
- Several errors render as one block with a compact list instead of one full-width red block each, and the header names the mode. A project with Blade-shipping vendors produces one render failure per source.
- Corrected doctor's unrenderable-source note: it said a bare CLI skips `.blade.php` sources, but with a renderer registered that cannot run outside its framework, it throws instead.

### Changed

- `docs/reference/cli.md` no longer tells Laravel projects to gate CI on `vendor/bin/boost sync --check`. The wrapper's command is the gate there, and the two CLIs do not share flag names.
- `PUBLIC_API.md` documents what an `@api` guarantee does not pin: the shape of a value as distinct from its type, raw array keys reached through a public property, and human-readable output. Code that pattern-matches an emitted path is coupled to something no promise covers, and it fails by matching nothing rather than erroring.

### Internal

- An architecture test guards partial reads of a failure channel — the shape behind most of the fixes above. It checks that every verdict renderer consults `hasErrors()`, that every emitter outcome reaches an operator, and that an error list covers both channels rather than one.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.7.0...1.8.0

## [1.7.0](https://github.com/sandermuller/boost-core/compare/1.6.1...1.7.0) - 2026-08-19

<!-- verified-sha: 939e048fdf32f7af54f285c4c40e2f82bf5fb7e6 -->
Antigravity joins the fan-out, and the family gets a documentation site.

### Added

- **Antigravity is a selectable agent.** `Agent::ANTIGRAVITY` reads the two paths Codex and Copilot already write — root `AGENTS.md` for guidelines and `.agents/skills/<name>/SKILL.md` for skills — matching `laravel/boost`'s own Antigravity agent. Selecting it next to one of those agents adds no new file; it exists so an Antigravity-only project can pick its own agent in `boost install` and receive the paths it reads, instead of having to know that "Codex" writes them.
  
  Antigravity publishes no committable command directory, so `boost sync` writes no command files for it. `boost doctor` says so: when `.ai/commands/` is populated and Antigravity is selected, the **Command-emit limitations** section now names it alongside Codex and Gemini and points at the skill route, which is the surface boost does write for it. Author the workflow as a skill and Antigravity picks it up.
  
- **A documentation site for the whole boost family.** [https://sandermuller.github.io/boost-core/](https://sandermuller.github.io/boost-core/) carries the guide, one section per family package, and the full CLI and configuration reference. The README is now the entry point rather than the manual.
  

### Internal

- The `Agent` enum backing values stay additive, as `PUBLIC_API.md` promises: `antigravity` is new, nothing existing moved. A wrapper resolving `list<Agent>` through `Agent::target()` needs no change.
- The end-to-end suite regained PHP 8.3 compatibility. It had picked up PHP 8.4's parenthesis-free constructor chaining in fifteen places, which parses only on 8.4 — the declared floor is `^8.3`, so both 8.3 CI legs could not run the suite at all. Shipped code was never affected.
- CI hardening: workflow checkouts drop the persisted git credential where nothing pushes, a `zizmor` job audits every workflow on each change, and Dependabot waits seven days before proposing a freshly published version.

### Upgrading

Nothing to do. No public API change and no configuration change. To use the new agent, add `Agent::ANTIGRAVITY` to `withAgents([...])` — or re-run `boost install` and pick it from the list.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.6.1...1.7.0

## [1.6.1](https://github.com/sandermuller/boost-core/compare/1.6.0...1.6.1) - 2026-08-15

<!-- verified-sha: ba06630a3212c933d5ac1582e44623e59aa26064 -->
A one-line correction to advice 1.6.0 shipped. `boost doctor` tells you how to stop `laravel/boost` re-seeding your guidance files, and which half of that you have to do by hand depends on the `sandermuller/project-boost-laravel` version installed. The threshold was set one release too low.

### Fixed

- **`boost doctor` named the wrong wrapper version for the `boost.json` hand-off.** Retiring `boost.json` — which is what stops `boost:update`, and therefore the `herd link` trigger, from re-seeding — ships in **project-boost-laravel 1.3.0**; `src/Coexistence/` and the `--keep-boost-json` flag are absent from the 1.2.0 tag. 1.6.0 gated the advice at `1.2.0`, so a project pinned to 1.2.x was told a successful `project-boost:sync` archives the file for it and could sit waiting for something that never happens. It now gets the manual instruction, exactly as it did before 1.6.0. Projects on 1.3.0 or later are unaffected, and both branches of the advice are pinned by tests.

### Upgrading

Nothing to do. No public API change, no configuration change, and no change to what `boost sync` writes.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.6.0...1.6.1
...
A coexistence release. On a project that also runs `laravel/boost`, both tools write into the same agent skill directories — and boost-core's post-sync sweep deleted whatever it had not just written, which meant laravel/boost's bundled skills, on every sync. Nothing said so. Meanwhile `herd link` reinstalled them behind your back, so the two tools took turns undoing each other.

### Fixed

- **Sync no longer deletes — or adopts — files another tool wrote into a boost-managed directory.** The post-sync sweep treated everything under a boost-managed `.gitignore` pattern that the run had not rewritten as stale. `laravel/boost` installs its bundled skills as real directories inside those same agent skill directories, so each `boost sync` removed them silently, and `herd link` put them straight back (it runs `php artisan boost:update` on its own whenever `vendor/laravel/boost` is present). The sweep is now gated on the ownership manifest: **boost deletes only what it owns.** Anything else is preserved and reported in one aggregated INFO. `sync --check` matches, so a CI drift check on a coexistence project stays green.
  
  The manifest write is filtered the same way. It used to record every file found under the managed directories, which adopted a preserved foreign file as boost's own output and let the *next* sync delete it — preservation was a one-sync reprieve. Only paths this sync wrote, paths boost already owned, and wrapper-claimed paths are recorded now.
  
  Three carve-outs keep the sweep honest. A project with no manifest file at all (a first sync, or `withGitignoreManagement(false)`) keeps the previous clean-slate behaviour, since nothing there can prove a file is not boost's own leftover. An existing but entry-less manifest still gates — "boost owns nothing right now" must not re-arm deletion. And a managed-ignore pattern naming a file exactly (`AGENTS.md`, not `dir/`) remains boost's own proof of authorship, so a de-selected agent's stale guidance is still reaped.
  

### Added

- **`boost doctor` explains the whole coexistence surface.** When `laravel/boost` is installed, the section now maps the two command families that share the `boost` name — `php artisan boost:*` (laravel/boost), `vendor/bin/boost *` (boost-core), `php artisan project-boost:*` (the wrapper) — and names the trigger behind "my files changed on their own": `herd link` runs `php artisan boost:update` by itself whenever `vendor/laravel/boost` is present, re-seeding guidance and skills with nobody typing a boost command.
  
  It also reports what that coexistence leaves on disk: skill directories in boost-managed locations that boost-core does not own (they reach one agent only and never enter the fan-out), skill targets that are symlinks into the source tree (the agent then reads raw source instead of the rendered, tag-filtered output, and `sync --check` shows no drift), and host-authored `.ai/skills/<name>` directories whose name `laravel/boost`'s `boost.json` also claims — it treats that path as its own canonical copy and overwrites it with vendor content on the next install.
  
  Finally it spells out how to stop the automatic re-seed for good, since `.ai/skills/` is also laravel/boost's user-skill directory and its mere existence keeps `boost:update` working regardless of what `boost.json` lists.
  
- **`boost install` adopts laravel/boost's agent list.** Setting boost-core up on a project that ran `php artisan boost:install` first no longer means re-picking the same agents from memory: the picker pre-selects the ones recorded in `boost.json` (`claude_code` maps to `claude-code`; agents boost-core has no case for are named rather than dropped silently). Deliberately narrow — it fires only when `boost install` scaffolds the config, so an existing `withAgents([])` is never repopulated. Nothing is written without confirmation; these are picker defaults.
  
- **Bundled `boost-command-surfaces` skill.** Ships to projects that allowlist `sandermuller/boost-core`. It gives agents the command map, the rule for which sync to run, the three similarly-named config files (`boost.json`, `config/boost.php`, `boost.php`) and what each one is for, plus the reasons guidance files and skills change on their own — the confusion that had agents running `php artisan boost:install` in a boost-core project.
  

### Internal

- `boost doctor` also explains the `.ai/rules` split: laravel/boost owns that directory, its guideline tells agents to read `.ai/rules/index.md` before editing, and its `record-rule` tool writes there. boost-core neither reads nor composes those files — they stay path-scoped and read on demand — and the instruction to read them travels only with laravel/boost's own guideline, so a project without the Laravel wrapper never learns the directory exists.
- `SyncManifest` gained `hasUnder()` and `existsForProjectRoot()`; the manifest-claim filter lives in `ManagedFileOps::claimableManagedFiles()`. All `@internal`.

### Upgrading

Nothing to do. No public API change and no configuration change. On a project that also runs `laravel/boost`, the first sync after upgrading stops deleting its bundled skill directories and reports what it preserved — run `boost doctor` for the full division of labour.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.5.0...1.6.0

## [1.5.0](https://github.com/sandermuller/boost-core/compare/1.4.2...1.5.0) - 2026-08-07

<!-- verified-sha: 4e40e4bf782d0c1e4aedf491f070fd2ec06e2e71 -->
Remote skill sources have been fetchable since 1.3, but declaring them was blind work: you had to already know a repo's skill names or its subdirectory layout before you could write a `withRemoteSkills()` entry. `boost remote` reads the repo first and lets you pick.

### Added

- **`boost remote [<owner>/<repo>]` — pick remote skills instead of declaring them blind.** The command resolves the ref, works out whether the repo publishes `.skill` release assets (bundle mode) or skill directories (path mode), reads each skill's `SKILL.md` frontmatter, and presents a checklist showing every skill with its description, its tags, and any clash with a skill the project already receives — a host `.ai/` skill that would shadow it, a vendor skill that would make the sync fail, or a `withExcludedSkills()` entry already blocking it. What you check is written to `boost.php`; what you uncheck is removed. The repo argument takes a bare `<owner>/<repo>` slug or any GitHub URL, and is prompted for when omitted. `--ref=<tag|branch|sha>` pins the source; `--mode=bundle|path` overrides detection. A new source defaults to the moving ref `latest`, and a source already declared keeps whatever version it has — re-running the picker to tick one extra skill never un-pins production config.
- **Dependencies from the same repo are pulled in.** A remote skill's `metadata.boost-requires` can only be satisfied when the config names the dependency too: the ingester fetches the declared refs and nothing else, so `SkillDependencyResolver` has no candidate to rescue and the sync could only warn that the dependency is missing. The picker now closes the selection over `boost-requires` transitively within the repo, reports what it added and why, and warns about names the repo does not publish.
- **Missing tags are offered up front.** Remote skills are tag-filtered exactly like vendor skills, so declaring a tagged skill without the matching `withTags()` entry writes config that never produces a file. The command detects that gap and offers to add the tags in the same run.
- **What was downloaded is kept.** Discovery has to fetch and unpack the repo to read frontmatter at all; that content is pruned to the selection and promoted into the normal cache slot, so the `boost sync` the command offers to run afterwards needs no network. A moving ref's resolution is recorded alongside it. A cache that cannot be written is reported and skipped, never fatal.
- `RemoteFetcher::listReleaseAssets()` reads the release payload `fetchAsset()` already fetches, so listing a repo's bundles costs one request instead of one per asset.

### Changed

- **The starter `boost.php` now ships `->withRemoteSkills([])` and imports `RemoteSkillSource`.** Existing configs are untouched, and nothing about sync behaviour changes. Without the empty array present, the first entry has to be spliced into the fluent chain, and php-parser's insertion heuristic produces cramped output; with it, writing an entry is a clean in-place array rewrite and the import keeps the emitted call short instead of fully-qualified.
- The starter template moved out of `InstallCommand` into `SanderMuller\BoostCore\Config\ConfigScaffolder`, so `boost install` and `boost remote` scaffold identically rather than from two drifting copies. `InstallCommand::scaffoldTarget()` still exists and delegates to it.

### Internal

- `withRemoteSkills([...])` is edited element-wise by a dedicated AST writer alongside `BoostConfigWriter`, matched on `(source, mode)` and deliberately ignoring the version so a re-pin updates the entry rather than appending a second one for the same repo — the shape the ingester rejects as a collision. It refuses outright, writing nothing and naming the offending line, when the array holds an entry it cannot read back, such as a raw `new RemoteSkillSource(...)`.
- **Dropped the abandoned `rector/type-perfect` dev dependency.** `tomasvotruba/type-coverage` 2.3.0 — the package type-perfect's own abandonment notice points at — absorbed it and ships its config, so requiring both registered `MethodNodeAnalyser` twice and PHPStan refused to boot. Only CI hit it: `composer.lock` is not committed, so CI resolves fresh and picked up 2.3.0 the day it shipped while local installs stayed on the working combination. The type-perfect rules are unchanged; on the PHP 8.4 analysis job they now load through one package instead of two. The `^2.1` constraint is deliberately not raised, since 2.3.0 requires PHP 8.4 and the test matrix still covers 8.3.

### Upgrading

Nothing to do. `boost remote` is additive, no public API changed, and existing `withRemoteSkills()` entries keep working exactly as before.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/1.4.2...1.5.0

### Fixed

- **Sync no longer deletes — or adopts — files another tool wrote into a boost-managed directory.** The post-sync sweep treated everything under a boost-managed `.gitignore` pattern that the current sync didn't rewrite as stale. `laravel/boost` installs its bundled skills as real directories inside those same agent skill directories, so every `boost sync` silently deleted them — and `herd link` runs `php artisan boost:update` on its own whenever `vendor/laravel/boost` is present (Herd's valet CLI, verified in `Resources/valet/cli/app.php`), which put them straight back. The sweep is now gated on the ownership manifest: boost deletes only what it owns, preserves the rest, and reports one aggregated INFO naming what it kept. `sync --check` matches, so CI drift checks don't go red on a coexistence project. Two carve-outs: with no manifest FILE at all (first sync, unmanaged `.gitignore`) the pass stays clean-slate — an existing but entry-less manifest still gates, since "boost owns nothing right now" must not re-arm deletion of another tool's files, and a managed-ignore pattern naming a file exactly (`AGENTS.md`) is still boost's own proof of authorship, so a dropped agent's stale guidance is reaped as before. The manifest write is filtered to match: it used to record every file found under the managed directories, which adopted a preserved foreign file into boost's ownership and let the *next* sync delete it — only paths this sync wrote, already owned, or wrapper-claimed are recorded now.

### Added

- **`boost doctor` maps the two colliding `boost` command surfaces.** When `laravel/boost` is installed, the coexistence section now spells out which entry point owns which verb — `php artisan boost:*` (laravel/boost), `vendor/bin/boost *` (boost-core), `php artisan project-boost:*` (the wrapper) — names the sync to run for that project's layout, and flags the `herd link` → `boost:update` trigger that re-seeds guidance without anyone typing a boost command.
- **`boost doctor` flags foreign skill directories and contested skill names.** It lists skill directories in boost-managed locations that boost-core does not own (they reach only that one agent and never enter the fan-out), and says so plainly when there is no manifest yet (the just-`boost:install`ed project) instead of staying silent, and warns when a host-authored `.ai/skills/<name>` collides with a name in `laravel/boost`'s `boost.json` — laravel/boost treats that path as its own canonical copy and overwrites it with vendor content on the next `boost:install` / `boost:update`.
- **`boost doctor` explains the `.ai/rules` split.** laravel/boost's bundled guideline tells agents to read `.ai/rules/index.md` before editing, and its `record-rule` MCP tool writes there. boost-core composes `.ai/guidelines/` and `.ai/skills/` and has no `.ai/rules` pipeline — the rule files stay path-scoped and read on demand. When the directory exists, doctor says who owns it, notes that the instruction to read it only travels with laravel/boost's guideline (so a project without the wrapper never gets it), and points always-on guidance at `.ai/guidelines/`.
- **`boost doctor` states how to stop the automatic re-seed.** `.ai/skills/` is also laravel/boost's user-skill directory: its installer adopts every directory there and publishes it to its own agents (symlink or copy), bypassing boost-core's rendering and tag filtering, and that directory's mere existence keeps `boost:update` doing work regardless of `boost.json`'s `skills` list. On a wrapper project the clean exit is to delete `boost.json`: only `boost:install` and `boost:update` read it (the MCP server, boost-core, and the wrapper all do not), and `boost:update` refuses to run without it, so the `herd link` trigger becomes a no-op. On a wrapper from 1.2 on, doctor instead notes that `project-boost:sync` removes the file for you (`--keep-boost-json` opts out). Below that, doctor says to delete it by hand, with the narrower `"guidelines": false` + `"skills": []` + `->withSkillsPath(...)` alternative for projects that want to keep the file — and warns that a project without the wrapper must keep `boost.json`, since nothing else would deliver laravel/boost's bundled content. Stated as options; no default changed.
- **`boost doctor` warns about symlinked skill targets.** laravel/boost links `<agent skills dir>/<name>` straight at the source tree; boost-core never writes through a live symlink, so its rendered, tag-filtered output is silently displaced by the raw source while `sync --check` reports no drift.
- **`boost install` adopts laravel/boost's agent list.** On a project that ran `php artisan boost:install` first, the agent picker now pre-selects the agents recorded in laravel/boost's `boost.json` (its snake_case names map onto the enum; `claude_code` → `claude-code`), so adopting boost-core doesn't mean re-picking the same list from memory. Deliberately narrow: it fires only when `boost install` SCAFFOLDS the config — an existing `boost.php` is the operator's decision, including `withAgents([])`, which reads like "not adopted yet" but means every agent was turned off and must not be repopulated from laravel/boost's state. Nothing is written without confirmation; these are picker defaults. Agents boost-core has no case for (`antigravity`, `factory`, `grok_build`, `pi`, `zed`) are named as not carried over rather than dropped silently. This is the adoption step `project-boost:sync` waits for before retiring `boost.json`.
- **Bundled `boost-command-surfaces` skill.** Ships to projects that allowlist `sandermuller/boost-core`. It gives agents the command map, the "which sync do I run" rule, the editing rules for `.ai/` vs `boost.json`, and the reasons guidance files and skills change on their own — the confusion that made agents run `php artisan boost:install` in a boost-core project.

## [1.4.2](https://github.com/sandermuller/boost-core/compare/1.4.1...1.4.2) - 2026-07-29

<!-- verified-sha: b61dd50a6f1f239e7a58dabf7b918fda6576c411 -->
...
## [1.3.1](https://github.com/sandermuller/boost-core/compare/1.3.0...1.3.1) - 2026-07-04

<!-- verified-sha: 5f93b71e52846babf145d7b37681eb6ae76e3760 -->
### Fixed
...
## [1.3.0](https://github.com/sandermuller/boost-core/compare/1.2.1...1.3.0) - 2026-07-02

<!-- verified-sha: 794417116f8e88a444c81a1ac5f9ecefe45a00a9 -->
...
**Upgrade note:** none — a strict minor over 1.2.x. Skills without companion files sync byte-identically to before. Skill packages that adopt asset siblings should floor their boost-core constraint at `^1.3`. No breaking change to configuration, CLI, hooks, or the 1.0 `@api` surface.
...
## [1.2.0](https://github.com/sandermuller/boost-core/compare/1.1.2...1.2.0) - 2026-06-19

<!-- verified-sha: REPIN-TO-GREEN-MAIN-SHA -->
### Added
...
## [1.1.0](https://github.com/sandermuller/boost-core/compare/1.0.0...1.1.0) - 2026-06-05

<!-- verified-sha: 930fd975604311704fe1d55873e5e6a2255ef9a3 -->
...
## [1.0.0](https://github.com/sandermuller/boost-core/compare/0.23.3...1.0.0) - 2026-06-04

<!-- verified-sha: 7a8db9896746f2fb78be8a622241774553b296e2 -->
The first stable release. boost-core's public surface is now under [Semantic Versioning](https://semver.org): from here on, breaking changes to it land only in a MAJOR bump, and MINOR/PATCH releases stay backward-compatible. 1.0.0 freezes exactly the surface 0.23.3 shipped — there is no behavioral change to the documented API. The committed surface is enumerated in [`PUBLIC_API.md`](PUBLIC_API.md).

### The committed surface

The SemVer promise covers the public surface only:

- **Config authoring API** — `BoostConfig::configure()`, the `BoostConfigBuilder` `with*()` methods, the `Agent`/`Tag` enums, and `RemoteSkillSource`.
- **CLI** — the `bin/boost` command names, their documented options, and exit codes (`0` ok, `1` failure, `2` usage).
- **Composer hooks** — `BoostAutoSync::run` / `runWithSummary`; new parameters are always optional-with-default.
- **Plugin contracts** — `FileEmitter`, `SkillRenderer`, `BoostWrapperContract` and their DTOs, locked at 1.0 (parameterless constructors; a contract method signature change is a major bump).

Everything marked `@internal` (the whole engine) and on-disk regenerable state (the sync manifest, remote-skill ledger, user-scope manifests, the `.boost/` ⁄ `.config/boost/` runtime dir, cache sentinels) is explicitly NOT covered and may change in any release.
...
- The `@internal` `UnrenderableSourceScanner` now returns `UnrenderableSource` value objects (path + message) so callers classify on the real extension rather than parsing the message; the symlink scanner exposes a shared `cleanupRootsFor()` used by both commands. Six regression tests cover the fixes above.


---

**Upgrade note:** none — a drop-in upgrade from 0.23.3. 1.0.0 introduces no breaking change; it promotes the existing surface to a SemVer-stable contract. See [`PUBLIC_API.md`](PUBLIC_API.md) for the committed surface and [`UPGRADING.md`](UPGRADING.md) for future migrations.

**Full Changelog**: https://github.com/SanderMuller/boost-core/compare/0.23.3...1.0.0

## [0.23.3](https://github.com/sandermuller/boost-core/compare/0.23.2...0.23.3) - 2026-06-04

<!-- verified-sha: 1a8ef4a130ca823ebcb9fb76532a1228cff8270a -->
### Documentation contract

- **The named-argument contract is now explicit.** Consumers pass named arguments to `@api` methods and construct `@api` value objects directly, so the PARAMETER NAMES are part of the frozen contract — a rename or reorder is breaking even when the class is unchanged. `PUBLIC_API.md` now pins the constructor parameters of `Skill` / `Guideline` / `RenderContext` / `WrittenFile` / `EmitterResult` / `Diagnostic` and the method parameters of `BoostSync::sync()` + `BoostConfig::load()`. Two value-shape pins are spelled out: `Diagnostic::$level` stays a string (`error`/`warning`/`info`, not enum-ified within `1.x`) and `SyncResult::$errors` stays `list<string>`.
...
## [0.23.1](https://github.com/sandermuller/boost-core/compare/0.23.0...0.23.1) - 2026-06-04

<!-- verified-sha: PENDING — pin to the green HEAD at cut time (after the cold-validations report; fold in any 0.23.0 regression they find) -->
<!-- DRAFT staged while the 0.23.1 tag is HELD for the fm0gwxtn/a4bg5vbh cold-validations, so any 0.23.0 regression batches into the same patch. -->
### Fixed
...
## [0.23.0](https://github.com/sandermuller/boost-core/compare/0.22.0...0.23.0) - 2026-06-04

<!-- verified-sha: 6c08d86f8cd09bfe6e8a566b9220d324cc3f944b -->
A dogfood-hardening release. A seven-package family adoption of 0.22 + the `.config/` layout — real production apps and the wrapper stack — surfaced one data-loss bug, one `composer update` footgun, a stranded `@api` seam, and two cases of `boost doctor` giving misleading (one actively destructive) advice on wrapper projects. 0.23.0 fixes all of them. Additive — no API break; a strict superset of the 0.22 surface.

### Added

- **`BoostConfig::load()`'s exceptions are `@api`.** `BoostConfigNotFoundException`, `InvalidBoostConfigException`, and `AmbiguousBoostConfigException` are now part of the frozen surface, catchable by name — a `@throws` of an `@api` method is part of its contract, so a wrapper can give a tailored message per failure mode. `load()` now declares all three with explicit `@throws`.
- **`BoostTags` is `@api` — the canonical tag-parse seam.** `BoostTags::parse(array): array{0: list<string>, 1: bool}` + `declaresTags(array): bool` (pure static, the other half of the already-`@api` `FrontmatterParser`). A wrapper that injects skills/guidelines now computes the SAME `[tags, valid]` the engine does — including the fail-closed contract (a non-string `metadata.boost-tags` → `valid = false` → ships nowhere) — instead of reinventing the parse and diverging on the malformed case. `parseString()` stays the `@internal` lexer.
