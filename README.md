...
| `boost doctor`                       | Offline health check — config, remote sources, cache state, emitter status, leaked conventions tokens (0.16.0) | [Remote skill sources](#remote-skill-sources)               |
...
| `boost validate`                     | Validate boost.php's `->withConventions([...])` against allowlisted vendors' schemas + scan emitted files for leaked conventions tokens (0.16.0) | [Project Conventions](#project-conventions)             |
...
| `boost where --conventions`          | Effective resolved slots + provenance (declared / schema-default / missing); `--json` for a machine-readable shape (0.15.0) | [Project Conventions](#project-conventions) |
