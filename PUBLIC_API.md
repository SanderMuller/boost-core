...
- `SanderMuller\BoostCore\Contracts\FileEmitter` + `SanderMuller\BoostCore\Sync\SyncContext` (param) + `SanderMuller\BoostCore\Sync\EmittedFile` (return) — emit custom files during sync. Parameterless constructors only.
...
- `SanderMuller\BoostCore\Contracts\BoostWrapperContract` + `SanderMuller\BoostCore\Agents\AgentTarget` — wrapper packages compute their emit surface against these.
