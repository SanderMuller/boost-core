...
**Branch-alias convention.** Every repo's `extra.branch-alias.dev-main` matches the minor line currently being shipped, as `<minor>.x-dev`, and is bumped when a new minor is cut. It was found at `1.4.x-dev` while 1.8 was being tagged — four minors stale, the same silent-drift shape as the `version` field below, and now checked by the same tag-push workflow. While shipping 0.9.x patches the alias is `0.9.x-dev`; the moment 0.10.0 is cut the alias bumps to `0.10.x-dev`. Consumers pinning `sandermuller/<pkg>: ^0.9` resolve to tagged 0.9.x releases via Packagist; consumers pinning `^0.9.0@dev` get dev-main via the alias.

**The static `version` field is for Dependabot — do not remove it, and keep it in step with the tag.** `composer.json` declares a `version` matching the most recent release. boost-core dev-requires `sandermuller/boost-skills`, which in turn requires `sandermuller/boost-core: ^1.4` — a self-reference through the family. Composer cannot modify the root package, so the root must already satisfy `^1.4` or NOTHING resolves. Locally and in CI the `COMPOSER_ROOT_VERSION: dev-main` env var supplies that; Dependabot has no equivalent knob and resolves the root as `1.0.0+no-version-set`, which fails every update it attempts — not only a boost-skills one. The field is the only lever that reaches Dependabot.

**BUMP IT ON EVERY RELEASE, to exactly the version being tagged.** The original note here claimed the field could stay at a floor because "Packagist reads the git tag and ignores this field". That is false, and it cost a release: Packagist COMPARES the field to the tag and skips the tag on a mismatch — `Skipped tag 1.8.0, tag (1.8.0.0) does not match version (1.4.0.0) in composer.json`. The 1.8.0 tag exists on GitHub and never reached Packagist, so `composer update` could not see it. Tags before 1.8.0 published fine only because the field did not exist yet. Any value still satisfies Dependabot's `^1.4` need, so matching the tag costs nothing and is the only value that also satisfies Packagist. `composer validate --strict` prints one warning about the field, which is expected and is not run in CI.
