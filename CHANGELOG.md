...
## [Unreleased](https://github.com/sandermuller/boost-core/compare/1.4.2...HEAD)

### Added

- **`boost remote <owner>/<repo>` — pick remote skills instead of declaring them blind.** The engine has fetched remote skills since 1.3, but the config side was hand-written: you had to already know a repo's skill names or subdirectory layout before you could put them in `withRemoteSkills()`. The new command reads the repo first. It resolves the ref, detects whether the repo publishes `.skill` release assets (bundle mode) or skill directories (path mode), reads each skill's frontmatter, and presents a checklist with descriptions, tags, and annotations for anything that would shadow a host skill, collide with a vendor skill, or sit outside `withExcludedSkills()`. The selection is written to `boost.php`; unchecking removes an entry. `--ref` pins the source, `--mode` overrides detection. The repo argument accepts a bare slug or any GitHub URL, and is prompted for when omitted.
- **Dependency closure at pick time.** A remote skill's `boost-requires` is only satisfiable when the config names the dependency too — `RemoteSkillIngester` fetches declared refs and nothing else, so sync could only warn. The picker now pulls transitive dependencies from the same repo into the selection and reports the ones it can't satisfy.
- **Tag-gap prompt.** Remote skills are tag-filtered like vendor skills, so declaring a tagged skill without the matching `withTags()` entry writes config that never produces a file. The command detects that and offers to add the missing tags in the same run.
- **Discovery primes the cache.** What the picker downloaded is pruned to the selection and promoted into the normal cache slot, so the `boost sync` it offers to run afterwards needs no network. A moving ref's resolution is recorded too. A cache that can't be written is reported, not fatal.
- `RemoteFetcher::listReleaseAssets()` — reads the release payload `fetchAsset()` already fetches, so listing a repo's bundles costs one request rather than one per asset.
- Element-wise editing of `withRemoteSkills([...])`, matched on `(source, mode)` and ignoring the version so a re-pin updates rather than appends. Refuses outright (writing nothing, naming the line, handing back a snippet to paste) when the array holds an entry it cannot read back, such as a raw `new RemoteSkillSource(...)`. Unlike every other array in `boost.php`, this one is rewritten per element rather than wholesale, so it lives in its own AST writer alongside `BoostConfigWriter`.
