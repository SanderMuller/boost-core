...
- **SKILL.md / guideline frontmatter keys boost-core CONSUMES** — `name`, `description`, `metadata.boost-tags`, `metadata.schema-required`, `argument-hint`. boost-core reads these from authored frontmatter; their names + semantics are frozen (a vendor authors against them, so renaming/dropping one silently changes behavior).
